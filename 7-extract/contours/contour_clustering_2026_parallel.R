...
#
# MODIFICATIONS (relative to original):
# 1. Parallel processing added for f0 measurement, distance matrix computation, and evaluation loops.
#    Uses doParallel/foreach. On Linux HPC systems, FORK clusters are used automatically for best performance.
#    Core count is read from SLURM_CPUS_PER_TASK if available, otherwise detectCores()-1.
# 2. "Skip cleaning" radio button added to Settings tab. When enabled, the cleaning step is bypassed
#    for both the TextGrid/measurement path and the file upload path.
#
...
# Determine number of cores to use (respects SLURM allocations on HPC clusters)
get_n_cores <- function() {
  slurm_cores <- suppressWarnings(as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", unset = NA)))
  if (!is.na(slurm_cores) && slurm_cores > 1) return(slurm_cores)
  max(1L, parallel::detectCores(logical = FALSE) - 1L)  # physical only
}

# Use FORK on Linux/Mac (much faster - workers inherit env without copying),
# fall back to PSOCK on Windows
make_cluster <- function(n) {
  cluster_type <- if (.Platform$OS.type == "unix") "FORK" else "PSOCK"
  parallel::makeCluster(n, type = cluster_type)
}
...
server <- function(input, output, session) {
  ...
  cat(paste0("Parallel cores available: ", get_n_cores(), "\n"))
    ...
    fillPage(
      ...
      sidebarPanel(
        ...
        tags$div(
          ...
          radioButtons("app.upl_lim", "File upload limit (MB):",choices = c("5" = 5,"10" = 10,"20" = 20,"100" = 95.368,"1000" = 1000), selected = app.fileinput(), inline = F, width = '100%')
        )
      ),
      sidebarPanel(
        width = 2,
      ...
      # NEW: Skip cleaning radio button
      sidebarPanel(
        width = 2,
        tags$div(
          title = "Skip the data cleaning step when loading a dataset you know is already clean. A warning notification will be shown when cleaning is skipped.",
          radioButtons(
            "app.skip_clean",
            HTML("<b>Data cleaning:</b>"),
            choices = list("Run cleaning" = 0, "Skip cleaning" = 1),
            selected = 0,
            inline = F,
            width = "100%"
          )
  ...
  # NEW: observer for skip cleaning setting
  observeEvent(input$app.skip_clean, {
    msg <- ifelse(input$app.skip_clean == 1, "will be skipped", "will run")
    showNotification(paste0("Data cleaning: ", msg))
    logging(paste0("Skip cleaning set to: ", input$app.skip_clean == 1))
  })
  ...
  obs.upload <- observeEvent(input$upload, {
    ...
    updateSelectInput(session, inputId = "cols.filename", choices = clms, selected = "filename")
    updateSelectInput(session, inputId = "cols.start", choices = clms, selected = "start")
    updateSelectInput(session, inputId = "cols.end", choices = clms, selected = "end")
    updateSelectInput(session, inputId = "cols.speaker", choices = c("[map later]",clms), selected = "speaker")
    updateSelectInput(session, inputId = "cols.interval_label", choices = clms, selected = "interval_label")
    updateSelectInput(session, inputId = "cols.stepnumber", choices = clms, selected = "stepnumber")
    updateSelectInput(session, inputId = "cols.f0", choices = clms, selected = clms[which(str_detect(clms,"f0"))[1]])
    updateSelectInput(session, inputId = "cols.cc.int.dB", choices = as.list(c(" ", colnames(df.u))), selected = clms[which(str_detect(clms,"int.dB"))[1]])
    updateSelectInput(session, inputId = "cols.cc.dur.s", choices = as.list(c(" ", colnames(df.u))), selected = clms[which(str_detect(clms,"dur"))[1]])
    updateSelectInput(session, inputId = "cols.steptime", choices = as.list(c(" ", colnames(df.u))), selected = clms[which(str_detect(clms,"steptime"))[1]])
    updateSelectInput(session, inputId = "cols.jumpkilleffect", choices = as.list(c(" ", colnames(df.u))), selected = "jumpkilleffect")
    updateSelectInput(session, inputId = "cols.add1", choices = as.list(c(" ", colnames(df.u))), selected = "")
    updateSelectInput(session, inputId = "cols.add2", choices = as.list(c(" ", colnames(df.u))), selected = "")
    updateSelectInput(session, inputId = "cols.add3", choices = as.list(c(" ", colnames(df.u))), selected = "")
    updateSelectInput(session, "cols.f0.scale",
    ...
    output$sel_filenames <- renderUI({
      if(is.null(input$cols.filename)){
        ...
        tags$div(title = "Select filename(s). Leave blank to select all.",
                 selectInput("cols.filename.sel", "Select filename(s):",
                             choices = unique(df.u[[input$cols.filename]]), selected = c(), multiple = T, width = '100%'))
      }
    })
    output$sel_ints <- renderUI({
      if(is.null(input$cols.interval_label)){
        ...
        tags$div(title = "Select interval label(s). Leave blank to select all.",
                 selectInput("cols.interval.sel", "Select interval label(s):",
                             choices = unique(df.u[[input$cols.interval_label]]), selected = c(), multiple = T, width = '100%'))
      }
    })
  })
  ...
  obs.cols.speaker <- observe({
    if (is.null(input$cols.speaker)) {
      ...
      if (input$cols.filename == "" | input$cols.start == "" | input$cols.end == "" |
          input$cols.speaker == "" | input$cols.interval_label == "" |
  ...
  obs.map_cols <- observeEvent(input$map_cols, {
    ...
    # MODIFIED: check skip_clean setting when routing after column mapping
    if(input$cols.speaker=="[map later]"){
      ...
      if(input$app.skip_clean == 1){
        # Skip cleaning - will navigate to Representation after column mapping is done
      }else{
    ...
    cbind(
      ...
      "steptime" = df.u[[input$cols.steptime]],
    ...
    for (n in c("cc.int.dB", "cc.dur.s", "steptime", "jumpkilleffect", "add1", "add2", "add3")) {
      if (nchar(input[[paste0("cols.", n)]]) != 0 && input[[paste0("cols.", n)]] != " ") {
    ...
    # MODIFIED: route to Representation if skipping cleaning
    if(input$cols.speaker != "[map later]" && input$app.skip_clean == 1){
      showTab(inputId = "tabset_data", target = "Representation")
      updateNavbarPage(session, "tabset_data", selected = "Representation")
      logging("Data cleaning skipped (user setting).")
      showNotification("Cleaning skipped. Proceeding to Representation.", type = "warning")
    }
  })
  ...
  obs.read_dir <- observeEvent(input$read_dir, {
    ...
    if (dir.exists(inDir()) == F) {
      ...
      if (input$snd_ext == 1) { snd.ext(".wav") }
      if (input$snd_ext == 2) { snd.ext(".mp3") }
      ...
      withProgress(message = "Reading folder ", {
        for (g in file_names) {
          incProgress(1 / ((length(file_names) * 1.2)),detail = g)
      ...
      output$file_names <- renderUI(fluidPage(
        HTML('<br>'),
        HTML('<b>Filenames for which audio and TextGrid were found in folder:</b>', '<br>'),
        checkboxInput(inputId = "sel_filenames_switch", label = "Select all/none", value = T),
        HTML('<br>'),
        fluidRow(
          column(width = 2),
          column(width = 7, align = "left",
                 checkboxGroupInput("sel_filenames", label = NULL,
                                    choices = unique(df.g$name_trim), selected = unique(df.g$name_trim)))
        )
      ))
      output$sel_tier <- renderUI({
        span(
          tags$hr(),
          div(title = "Select tier(s) from the available interval tiers in the TextGrids.",
              isolate(
                selectInput("sel_tier", "Select interval tier(s) for analysis:",
                            choices = unique(df.g$tier_name), selected = 1, multiple = T, width = '100%'))),
          div(title = "Select interval(s) from the selected interval tier(s) in the TextGrids. Leave blank to select all non-empty.",
              selectInput("sel_interval", "Select interval label(s) (optional):",
                          choices = c(), selected = 1, multiple = T, width = '100%')),
        )
      })
    }
  })
  ...
  obs.sel_filenames_switch <- observeEvent(input$sel_filenames_switch, {
    if (input$sel_filenames_switch == T) {
      updateCheckboxGroupInput(session, inputId = "sel_filenames", selected = unique(df.g$name_trim))
    } else {
  ...
  obs.sel_tier <- observeEvent(input$sel_tier, {
    ...
    updateCheckboxGroupInput(session, inputId = "sel_filenames",
  ...
  obs.to_measures <- observeEvent(input$to_measures, {
    ...
    if (nrow(subset(df.g, tier_name %in% c(input$sel_tier) & text %in% int_text & name_trim %in% input$sel_filenames)) == 0) {
      showNotification("With the current selection there are no intervals left for analysis.", type = "error")
    } else {
      subset(df.g, tier_name %in% c(input$sel_tier) & text %in% int_text & name_trim %in% input$sel_filenames) ->> df.g
  ...
  obs.disp_sample <- observeEvent(input$disp_sample, {
    ...
    if(dir.exists("www")==F){ dir.create("www") }
    ...
    plotServerList <- lapply(1:length(isolate(m.rows())), function(i) {
      plotServer(paste0("plot", i), df.g$name_trim[isolate(m.rows()[i])],
                 round(df.g$xmin[isolate(m.rows()[i])], 3), isolate(npoints()), input$f0.min, input$f0.max)
    })
  ...
  obs.do_measure <- observeEvent(input$do_measure, {
    ...
    if(any(input$incl_f0, input$incl_int, input$incl_dur)==F){
      ...
      # MODIFIED: check skip_clean setting when routing after measurement
      if(input$app.skip_clean == 1){
        showTab(inputId = "tabset_data", target = "Representation")
        updateNavbarPage(session, "tabset_data", selected = "Representation")
        logging("Data cleaning skipped after measurement (user setting).")
        showNotification("Cleaning skipped. Proceeding to Representation.", type = "warning")
      }else{
  ...
  obs.spk.cols <- observeEvent(input$spk.cols,{
    seps(unique(as.vector(str_extract_all(as.vector(unlist(subset(df.l, select = input$spk.cols),)), '[[:punct:]]',simplify = T))))
    updateSelectInput(session,"spk.sep",choices = c(" ", seps()))
  })
  
  obs.spk.mapcol <- observeEvent(input$spk.mapcol, {
    as.vector(
      str_split(string = df.l[,get(input$spk.cols)], pattern = input$spk.sep,simplify = T)
      [,which(colnames(as.data.table(str_split(string = df.l[,get(input$spk.cols)], pattern = input$spk.sep, simplify = T)))==input$spk.sepcol)]
    ) ->> df.l$speaker
    logging("Speaker column added.")
    showNotification("Speaker column mapped.")
    removeTab(inputId = "tabset_data", target = "Speakers")
    # MODIFIED: check skip_clean after speaker mapping too
    if(input$app.skip_clean == 1){
      showTab(inputId = "tabset_data", target = "Representation")
      updateNavbarPage(session, "tabset_data", selected = "Representation")
      logging("Data cleaning skipped after speaker mapping (user setting).")
      showNotification("Cleaning skipped. Proceeding to Representation.", type = "warning")
    }else{
  ...
  observeEvent(input$cont.repr,{
    ...
    if(length(pc.cols)!=0){ subset(df.l, select = -pc.cols) ->> df.l }
  ...
  observeEvent(input$cc.vars,{
    if(any(is.na(subset(df.l, select = paste0("cc.",input$cc.vars))))){
      updateSelectInput(session, "cc.distm",
                        choices = list("euclidean (L2 norm = RMSE)" = "euclidean", "dynamic time warping" = "dtw_basic"),
                        selected = "euclidean")
    } else {
      updateSelectInput(session, "cc.distm",
                        choices = list("euclidean (L2 norm = RMSE)" = "euclidean", "mean absolute scaled error (MASE)" = "mase",
                                       "dynamic time warping" = "dtw_basic", "pearson (sqrt(2*(1-ρ)))" = "cor"),
                        selected = "euclidean")
    }
  })
  ...
  observeEvent(input$do.cc,{
    ...
    output$cl.sel <- renderUI({verticalLayout(HTML('<br>'),
                                              div(title = "Removes the selected clusters from the data.", actionButton("cl.rem","Remove clusters:",width = '100%')),
                                              HTML('<br>'),
  ...
  fileinput <- function() {
    output$upload <- renderUI({
      tags$div(
        ...
        fileInput("upload", label = NULL, accept = ".csv", buttonLabel = "Find .csv",
                  placeholder = paste0("requires long data file (max ",floor((app.fileinput() * 1024^2)/1000000)," MB)"), multiple = F)
      )
    })
  }
  
  f0.conv <- function(f0,from,to){
    if(from=="Hz" && to=="ST")  return(log10((f0 / 50)) * 39.87)
    if(from=="ST" && to=="Hz")  return(50*(2 ^ (f0/12)))
    if(from=="Hz" && to=="ERB") return(16.7*log10((0.006046*f0)+1))
    if(from=="ERB" && to=="Hz") return(165.4 * (10^(0.06*f0)-1))
  }
  ...
  make.df.l <- function() {
    ...
    for (r in m.rows()) {
      rbind(dfl,
            cbind("filename" = rep(df.g$name_trim[r], npoints()),
                  "start" = rep(df.g$xmin[r], npoints()),
                  "end" = rep(df.g$xmax[r], npoints()),
                  "speaker" = rep(NA, npoints()),
                  "interval_label" = rep(df.g$text[r], npoints()),
                  "steptime" = df.g$xmin[r] + cumsum(rep((df.g$xmax[r] - df.g$xmin[r]) / (npoints() + 1), npoints())),
                  "stepnumber" = 1:npoints())) -> dfl
    }
  ...
  # MODIFIED: parallel f0 measurement
  tsf0 <- function() {
    load.quiet("wrassp")
    
    n_obs <- nrow(df.l) / npoints()
    n_cores <- if (n_obs < 5000) 1L else get_n_cores()
    logging(paste0("tsf0: using ", n_cores, " parallel core(s)."))
    
    # Snapshot reactive values for use inside workers
    .inDir     <- inDir()
    .snd.ext   <- snd.ext()
    .npoints   <- npoints()
    .f0.min    <- input$f0.min
    .f0.max    <- input$f0.max
    .f0.timestep  <- input$f0.timestep
    .f0.fit       <- input$f0.fit
    .f0.smooth_bw <- input$f0.smooth_bw
    .df.g      <- df.g
    .m.rows    <- m.rows()
    ...
    results <- foreach::foreach(
      r = .m.rows,
      .packages = c("wrassp", "zoo", "pracma"),
      .export   = character(0)   # FORK inherits; PSOCK uses .packages
    ) %dopar% {
      f0.values <- rep(NA, .npoints)
      f0.track  <- rep(NA, .npoints)
      if ((.df.g$xmax[r] - .df.g$xmin[r]) > (.f0.timestep / 500)) {
        tryCatch({
          f0.object <- wrassp::mhsF0(
            paste0(.inDir, .df.g$name_trim[r], .snd.ext),
            beginTime  = .df.g$xmin[r],
            endTime    = .df.g$xmax[r],
            minF       = as.numeric(.f0.min),
            maxF       = as.numeric(.f0.max),
            windowShift= as.numeric(.f0.timestep),
            minProb    = as.numeric(.f0.fit),
            toFile     = FALSE
          )
          raw <- as.vector(ifelse(f0.object$pitch == 0, NA, f0.object$pitch))
          if (sum(is.na(raw)) != length(raw)) {
            f0.values <- stats::approx(raw, n = .npoints, na.rm = FALSE)$y
            f0.track  <- f0.values
            f0.values <- zoo::na.approx(f0.values, na.rm = FALSE)
            f0.values <- pracma::approxExtrap(1:.npoints, f0.values,
                                              xout = 1:.npoints, method = "constant")$y
            f0.values <- stats::ksmooth(1:.npoints, f0.values, kernel = "normal",
                                        bandwidth = .f0.smooth_bw, n.points = .npoints)$y
            f0.values <- round(f0.values, 3)
          }
        }, error = function(e) NULL)
      }
      list(f0 = f0.values, track = f0.track)
    }
    
    f0    <- unlist(lapply(results, `[[`, "f0"))
    track <- unlist(lapply(results, `[[`, "track"))
  ...
  tsint <- function() {
    ...
    withProgress(message = paste0("Intensity "), {
      for (r in m.rows()) {
        incProgress(1 / ((length(m.rows()) * 1.1)),detail = df.g$name_trim[r])
        rmsana(paste0(inDir(), df.g$name_trim[r], snd.ext()),
               beginTime = df.g$xmin[r], endTime = df.g$xmax[r], toFile = F)$rms -> int.values
        if(ncol(int.values)>1){ rowMeans(int.values) -> int.values }
  ...
  plotUI <- function(id) {
    ns <- NS(id)
    div(align = "center", style = "padding-bottom: 1em;", plotOutput(ns("plot"), width = "50%"))
  }
  
  playUI <- function(id) {
    ns <- NS(id)
    div(align = "center", style = "padding-bottom: 1em;", uiOutput(ns("play"), width = "50%"))
  }
  ...
  plotServer <- function(id, f, t, n, i, a) {
    moduleServer(id, function(input, output, session) {
      output$plot <- renderPlot({
        ggplot(mapping = aes(x = df.l$steptime[df.l$filename == f & df.l$start == t])) +
          geom_line(mapping = aes(y = df.l$track[df.l$filename == f & df.l$start == t]), na.rm = TRUE) +
          geom_line(mapping = aes(y = df.l$cc.f0.Hz[df.l$filename == f & df.l$start == t]),
                    linetype = "dashed", colour = "#FF0000", na.rm = TRUE) +
          coord_cartesian(ylim = c(i, a)) +
          labs(title = paste0("filename: ", f),
               subtitle = paste0("interval label: ", df.l$interval_label[df.l$filename == f & df.l$start == t][1]),
               x = "Interval time (s)", y = "f0 (Hz)") -> p
        if (sum(is.na(df.l$cc.f0.Hz[df.l$filename == f & df.l$start == t])) == n) {
          p + annotate("text",
                       x = df.l$steptime[df.l$filename == f & df.l$start == t][floor(n / 2)],
                       y = mean(c(i, a)), label = "No reliable f0 detected.", colour = "#FF0000", size = 7) -> p
        }
  ...
  check.clean <- function() {
    ...
    if("cc.f0.Hz" %in% colnames(df.l)){
      ...
      0 -> d.f0; 0 -> na.f0; 0 -> neg.f0
      paste0("0 problems found in f0 values (none in data)","\n") -> f0.prb
    }
    if(sum(na.f,na.s,na.f0,neg.f0,na.i,d.f0)==0){
      output$clean.set <- renderUI({div(title = "Max N velocity exceedings.", isolate(numericInput("clean.set","Max N velocity exceedings",value = 0,min = 0,max = max(df.l$stepnumber,na.rm = T)-1,step = 1)))})
      output$clean.recheck <- renderUI({div(title = "Clean and re-check data.", actionButton("clean.recheck","Clean and re-check",disabled = T))})
      output$clean.cont <- renderUI({actionButton("clean.cont", "Continue with cleaned data")})
      output$clean.ign <- renderUI({NULL})
    } else {
      output$clean.set <- renderUI({div(title = "Max N velocity exceedings.", isolate(numericInput("clean.set","Max N velocity exceedings",value = 0,min = 0,max = max(df.l$stepnumber,na.rm = T)-1,step = 1)))})
      output$clean.recheck <- renderUI({div(title = "Clean and re-check data.", actionButton("clean.recheck","Clean and re-check"))})
      output$clean.cont <- renderUI({actionButton("clean.cont", "Continue with cleaned data",disabled = T)})
      output$clean.ign <- renderUI({div(title = "This enables to continue without cleaning and is likely to produce errors and app crashes. Not recommend.", checkboxInput("clean.ign","Ignore problems",F))})
    }
    removeNotification("check.clean", session)
    return(list(
      text = paste0("Data summary:","\n","=============","\n",
                    unq.f, " filenames","\n", unq.s, " speakers","\n",
  ...
  do.fpca <- function(c){
    if(input$f0.Npc!=0){
      ...
      load.quiet("fda")
      df.p <- c()
      for (p in 1:npoints()){ rbind(df.p, df.l[[c]][df.l$stepnumber==p]) -> df.p }
      list(id = colnames(df.p), cue = as.matrix(df.p)) -> df.p
      cue_basis   = fda::create.bspline.basis(breaks = 1:npoints())
      harmLfd_cue = fda::int2Lfd(2)
      cuefdPar    = fda::fdPar(cue_basis,harmLfd_cue,input$f0.Lbd)
      cuefd       = fda::smooth.basis(1:npoints(),df.p$cue,cuefdPar)
      cuepca      = fda::pca.fd(cuefd$fd,nharm=input$f0.Npc)
      harmfd      = cuepca$harmonics
      harmvals    = fda::eval.fd(1:npoints(),harmfd)
      ...
      ggplot(dataPCplot) +
        geom_line(aes(x=step,y=value)) + xlab("measurement point") + ylab("PC (harmonic) value") +
        facet_wrap(~ PC) + theme_bw(base_size = 20) + labs(title = gsub("pc.","", c)) -> p
  ...
  do.repr <- function(m){
    ...
    if(input$f0.deriv!="n"){ load.quiet("pracma") }
    ...
    if(input$f0.deriv=="d2"){
      for (rws in 1:(nrow(df.l)/npoints())) {
        pracma::gradient(pracma::gradient(df.l[[n.repr]][((rws*npoints())-(npoints()-1)):(rws*npoints())])) ->> df.l[[n.repr]][((rws*npoints())-(npoints()-1)):(rws*npoints())]
      }
    }
    if(input$f0.deriv=="d3"){
      for (rws in 1:(nrow(df.l)/npoints())) {
        pracma::gradient(pracma::gradient(pracma::gradient(df.l[[n.repr]][((rws*npoints())-(npoints()-1)):(rws*npoints())]))) ->> df.l[[n.repr]][((rws*npoints())-(npoints()-1)):(rws*npoints())]
      }
    }
    ...
    if(m=="cc"){
      logging(paste0("F0 representation added: ",gsub("cc.","",n.repr),
                     ifelse(input$f0.Npc!=0,paste0(", with ",input$f0.Npc," principal components ","(λ: ",input$f0.Lbd,"), selected PCs: ",paste0(sel.PCs,collapse = ", ")),"")))
    }
    return(list(colname = n.repr))
  }
  
  # MODIFIED: parallel distance matrix computation in do.cc
  do.cc <- function(n, eval){
    showNotification(id = "nt.cc",paste0("Performing clustering (n = ",n,") ..."),duration = NULL)
    vars <- if(eval==1) input$cc.vars[1] else input$cc.vars
    npoints(max(df.l$stepnumber))
    
    n_obs <- nrow(df.l) / npoints()
    n_cores <- if (n_obs < 5000) 1L else get_n_cores()
    logging(paste0("do.cc: using ", n_cores, " parallel core(s) for distance computation."))
    
    # Snapshot data for workers
    .df.l    <- df.l
    .npoints <- npoints()
    .distm   <- input$cc.distm
    
    cl <- make_cluster(n_cores)
    doParallel::registerDoParallel(cl)
    on.exit(parallel::stopCluster(cl), add = TRUE)
    
    # Compute one distance matrix per variable in parallel
    dist_vectors <- foreach::foreach(
      var = vars,
      .packages = c("stats", "proxy", "dtwclust", "Metrics", "usedist"),
      .export   = character(0)
    ) %dopar% {
      is_singleton <- grepl("PC|dur", var)
      m <- matrix(
        na.omit(.df.l[[paste0("cc.", var)]]),
        ncol  = ifelse(is_singleton, 1, .npoints),
        byrow = TRUE
      )
      d <- switch(.distm,
                  "euclidean" = stats::dist(m, method = "euclidean"),
                  "mase" = {
                    mase_distance <- function(r1, r2) Metrics::mase(r1, r2)
                    usedist::dist_make(m, mase_distance)
                  },
                  "dtw_basic" = {
                    stats::as.dist(proxy::dist(m, method = "dtw_basic", upper = FALSE))
                  },
                  "cor" = {
                    stats::as.dist(proxy::dist(m, method = "correlation", upper = FALSE))
                  }
      )
      dv <- as.vector(d)
      rng <- range(dv, na.rm = TRUE)
      if (rng[2] > rng[1]) (dv - rng[1]) / (rng[2] - rng[1]) else dv * 0
    }
    
    # Sum scaled distance vectors, reconstruct dist object
    combined_vec <- Reduce(`+`, dist_vectors)
    n_obs <- nrow(.df.l) / .npoints
    cc.d  <- structure(combined_vec, class = "dist",
                       Size = n_obs, Diag = FALSE, Upper = FALSE,
    ...
    if(input$cc.method=="hac"){
      ...
      ggdendro::ggdendrogram(hac, theme_dendro = T, labels = F) +
    ...
    if(input$cc.method=="pam"){
      load.quiet("cluster")
      hideTab(inputId = "tabs_cl", target = "Dendrogram")
      if(exists("dendrogram",envir = .GlobalEnv)) rm("dendrogram", envir = .GlobalEnv)
      dendro <- NULL
      cluster::pam(cc.d, k = n, pamonce = input$cc.opt)$clustering -> cluster
    }
    if(eval==1){
      removeNotification("nt.cc",session)
      return(as.data.frame(cbind(matrix(na.omit(.df.l[[paste0("cc.",vars[1])]]),
                                        ncol = .npoints, byrow = TRUE), cluster)))
    }
    rep(cluster, each = npoints()) ->> df.l$cluster
    do.proto()
    paste0(1:n, " (n=", as.vector(table(df.l$cluster)/npoints()), ")") -> labs
    if("cc.dur.s" %in% colnames(df.l) && any(grepl("dur", vars))){
      str_replace_all(labs, pattern = "\\)",
                      replacement = paste0(", d=",round(as.vector(unlist(
                        (df.l %>% group_by(cluster) %>% summarise(mean=mean(cc.dur.s,na.rm=T),median=median(cc.dur.s,na.rm=T)))[input$plot.vals])),2),")")) -> labs
    }
    attributes(labs)$names <- 1:n
    ggplot(df.l, aes(x = stepnumber, y = df.l[[paste0("cc.",cc.y1())]])) +
      stat_summary(fun = input$plot.vals, group = "cluster", geom = "line",
                   colour = app.yL.col(), linetype = app.yL.type(), linewidth = 1, show.legend = F) +
      {if(input$plot.vals=="mean")
        stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), group = "cluster",
                     geom = 'ribbon', alpha = .2, show.legend = F)} +
      {if(input$plot.vals=="median")
        stat_summary(fun.data = median_hilow, group = "cluster", geom = 'ribbon', alpha = .2, show.legend = F)} +
      facet_wrap(~cluster, ncol = input$plot.cols, labeller = as_labeller(labs)) +
      xlab("measurement point") + ylab(paste0(cc.y1()," (",input$plot.vals,")")) +
      theme(axis.title = element_text(size = 20), axis.text = element_text(size = 20),
            strip.text = element_text(size = 20)) -> p
    if(is.null(input$plot.y2)==F){
      if (input$plot.y2!="none"){
        if (input$plot.y2=="prototype"){
          p + geom_line(aes(y=df.l[[paste0("cc.",cc.y1())]][df.l$proto=="x"]),
                        data = subset(df.l, proto =="x"), stat = "identity",
                        colour = app.yR.col(), linetype = app.yR.type(), linewidth = 1, show.legend = F) -> p
        }else{
          load.quiet("scales")
          scales::rescale(x = df.l[[paste0("cc.",input$plot.y2)]], to = c(min(df.l[[paste0("cc.",cc.y1())]]), max(df.l[[paste0("cc.",cc.y1())]]))) -> y2
          ...
          p + scale_y_continuous(cc.y1(), sec.axis = sec_axis(~((. -a.min) * b.diff / a.diff) + b.min, name = input$plot.y2)) +
            stat_summary(mapping = aes(y = y2), data = df.l, fun = input$plot.vals, group = "cluster",
                         geom = "line", colour = app.yR.col(), linetype = app.yR.type(), linewidth = 1, show.legend = F) +
            {if(input$plot.vals=="mean")
              stat_summary(mapping = aes(y = y2), data = df.l, fun.data = mean_sdl, fun.args = list(mult = 1),
                           group = "cluster", geom = 'ribbon', alpha = .2, show.legend = F)} +
            {if(input$plot.vals=="median")
              stat_summary(mapping = aes(y = y2), data = df.l, fun.data = median_hilow,
                           group = "cluster", geom = 'ribbon', alpha = .2, show.legend = F)} -> p
        }
        p + theme(axis.title.y.left = element_text(colour = app.yL.col())) +
          theme(axis.title.y.right = element_text(colour = app.yR.col())) -> p
      }}
    savethis$c_plot <- p
    as.data.table(cbind(rep("",nclust()), 1:nclust(),
    ...
    logging(paste0("Clustering performed: ",n," clusters; ",nrow(df.l)/npoints()," observations, variables: ",
                   ...
                   ifelse(input$cc.method=="hac",paste0("; method: HAC; linkage: ",input$cc.link),
                          paste0("; method: PAM (optimizer: 'pamonce=",input$cc.opt,"')")),
                   "; distance metric: ",input$cc.distm))
  }
    ...
    if(do==1){
      output$cl.contab <- renderUI({
        if(is.null(df.l) || input$tabs_main!="Clustering"){
          ...
          span(
            fluidRow(
              column(width = 11, div(title = "Generate a contingency table.", HTML('<b>Contingency table</b>'), style = 'display: inline-block; vertical-align: bottom; font-size: 1.5em')),
              column(width = 1, span(div(title = "Save table as csv file to disk",downloadButton("dl.contab","Save"))))
            ),
            HTML('<br>'),
            fluidRow(
              column(width = 2, 
                     verticalLayout(
                       div(title = "Select the column in the datafile as variable from which to extract the levels.",
                       ...
                       renderUI(div(title = "Choose separator character to split the selected column.",
                                    selectInput("cl.contab.sep", label = "Separator:",
                                                choices = c(" ", unique(as.vector(str_extract_all(as.vector(unlist(subset(df.l, select = input$cl.contab.colx),)), '[[:punct:]]',simplify = T))))))),
                       renderUI({
                         if(is.null(input$cl.contab.sep)) return(NULL)
                         if(input$cl.contab.sep=="") return(NULL)
                         div(title = "Select which (split) column to use in the contingency table.",
              ...
              column(width = 8,
                     renderUI({
                       ...
                       if(input$cl.contab.ncol=="") return(NULL)
                       as.vector(str_split(string = df.l[,get(input$cl.contab.colx)], pattern = input$cl.contab.sep, simplify = T)
                                 [,which(colnames(as.data.table(str_split(string = df.l[,get(input$cl.contab.colx)], pattern = input$cl.contab.sep, simplify = T)))==str_split(input$cl.contab.ncol," '",simplify = T)[1])]) -> x
  ...
  # MODIFIED: parallel evaluation loop in do.eval
  do.eval <- function() {
    load.quiet("purrr")
    
    n_obs <- nrow(df.l) / npoints()
    n_cores <- if (n_obs < 5000) 1L else get_n_cores()
    logging(paste0("do.eval: using ", n_cores, " parallel core(s)."))
    
    eval_range <- isolate(input$eval.rg[1]):isolate(input$eval.rg[2])
    ...
    if (isolate(input$eval.method) == 1) {
      ...
      getNormalParameters <- function(df) {
        ...
        mns$fn <- "mean"; colnames(mns)[1] <- "cluster"
        aggregate(df[substr(names(df), 1, 1) != "c"], by = list(df$cluster), FUN = sd) -> sds
        colnames(sds)[1] <- "cluster"; sds$fn <- "sd"
        rbind(mns, sds)
      }
      ...
      getRelInfoOfToken <- function(row, parameters) {
        (1:npoints()) %>% sprintf("V%d", .) %>%
          purrr::map_dbl(function(Xfield) {
            getRelInfoOfSample(row[, Xfield], parameters[, c(Xfield, "fn")])
          }) %>% sum()
      }
      getRelInfoOfData <- function(df, parameters) {
        (1:length(df$cluster)) %>%
          purrr::map_dbl(function(ri) {
            getRelInfoOfToken(df[ri, ], parameters[parameters$cluster == df$cluster[ri], ])
          }) %>% sum()
      }
      getInfo <- function(v) {
        data.frame(Ct = 1, v = v) %>% aggregate(Ct ~ v, ., sum) -> w
        nlogn <- function(x) x * log(x)
        nlogn(sum(w)) - sum(nlogn(w))
      }
      .eval.smooth <- isolate(input$eval.smooth)
      mkInfoCost <- function(df) {
        df %>% getNormalParameters() -> params
        df1 <- df; df1$cluster <- 1
        df1 %>% getNormalParameters() -> paramsAll
        params1 <- params[params$fn == "mean", ]; params1$cluster <- 1
        getRelInfoOfData(params1, paramsAll) / .eval.smooth +
          getInfo(df$cluster) +
          getRelInfoOfData(df, params) / .eval.smooth
      }
      ...
      cost <- foreach::foreach(
        r        = eval_range,
        .combine = c,
        .packages = c("purrr"),
        .export  = c("do.cc", "npoints", "input", "df.l",
                     "mkInfoCost", "getNormalParameters", "precisionInterval",
                     "getRelInfoOfSample", "getRelInfoOfToken", "getRelInfoOfData",
                     "getInfo", "Epsilon")
      ) %dopar% {
        df <- do.cc(r, 1)
        npoints(ncol(df) - 1)
      ...
      cbind.data.frame(clusters = eval_range, cost = cost) ->> df.e
      ggplot(data = df.e) +
        geom_line(aes(x = clusters, y = cost)) +
        xlab("N clusters") + ylab("Information cost") +
        annotate(geom = "point",
                 x = df.e$clusters[which.min(na.omit(df.e$cost))],
                 y = min(df.e$cost, na.rm = T), size = 2) +
    ...
    if (isolate(input$eval.method) == 2) {
      ...
      wb_results <- foreach::foreach(
        ...
        for (c in 1:r) {
          w <- c()
          for (m in 1:(ncol(df)-1)) { append(w, sd(df[df$cluster == c, m])) -> w }
          append(wc, mean(w)) -> wc
        }
        within_val <- mean(na.omit(wc))
        bmvar <- c()
        for (m in 1:(ncol(df)-1)) {
          bm <- c()
          for (c in 1:r) { mean(na.omit(df[df$cluster == c, m])) -> b; append(bm, b) -> bm }
          append(bmvar, abs(max(bm) - min(bm))) -> bmvar
        }
        between_val <- mean(bmvar)
        c(within = within_val, between = between_val)
      }
      
      scales::rescale(wb_results[,"within"])  -> within
      scales::rescale(wb_results[,"between"]) -> between
      cbind.data.frame(clusters = eval_range, within = within, between = between) ->> df.e
      ggplot(df.e, aes(clusters)) +
        geom_line(mapping = aes(y = within,  linetype = "within")) +
        geom_line(mapping = aes(y = between, linetype = "between")) +
        xlab("N clusters") + ylab("Scaled variance") +
        scale_linetype_manual("Cluster variance", values = c("within" = 1, "between" = 2)) +
        theme_bw(base_size = 20) -> p
    }
    savethis$e_plot <- p
    logging(paste0("Evaluation performed: ", isolate(input$eval.rg[1]), "-", isolate(input$eval.rg[2]),
                   " clusters, variable: ", input$cc.vars[1],
                   ifelse(isolate(input$eval.method)==1, paste0(", MDL (bending: ",isolate(input$eval.smooth),")"), ", W/B var")))
    return(list(plot = p))
  }
  ...
  do.proto <- function() {
      ...
      for(cntr in which(df.l$cluster==c & df.l$stepnumber==1)){
        sqrt(mean((
          as.vector(unlist(lapply(split(na.omit(df.l[[paste0("cc.",cc.y1())]][df.l$cluster==c]),
                                        f = df.l$stepnumber[df.l$cluster==c & is.na(df.l[[paste0("cc.",cc.y1())]])==F]),
                                  FUN = input$plot.vals)))-
    ...
    if(exists("df.g")){
      ...
      output$proto.play <- renderUI({
        tagList(lapply(1:length(isolate(m.rows())), function(i) {
          fluidRow(span(playUI(paste0("play", i)),
                        div(align = "center", renderTable(savethis$p_tab[i,])),
                        HTML('<br><br>')))
        }))
      })
    }else{
